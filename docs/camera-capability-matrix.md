...
**界面按这张表判定，不按常量**：`PtpChannel.captureSupport` 读当前 `functionMode`——模式 0 给
`WRITABLE`（实测），模式 1 给 `UNKNOWN`（本表标未验证），UPnP 通道给 `UNSUPPORTED`（实测）。
这里原来是恒 `true`，等于把整卡模式那一行的「未验证」在界面上写成「可以拍」。
`UNKNOWN` 在遥控页显示为「当前模式下遥控拍摄没有实测记录，先别当成不支持」——
未知不等于不支持（已知坑 14），把它涂红只会把用户推去重连，而重连会让句柄全部失效。
