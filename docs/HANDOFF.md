...
30. **状态标签必须在动作确认之后才改**（批次 C）：旧的 `enter(mode)` 先把界面上的
    范围改成「存储卡」，再去判断传输中/执行通道切换，失败时标签已经改了而列表还是旧范围——
    界面从此在说一件没发生的事。顺序永远是：确认相机通道切过去了 → 再动标签、列表、选择集合。
...
34. **扫码从弹窗搬进页面时，会杀死连接的不是布局，是 `release()` 的时机**（批次 D）：
    ...
    手动 IP 两条路根本没发起配网请求，那一步必须显示「不适用」而不是已完成——
    拿用户自己做的事邀功，下一次失败就没人信界面上其他话了。
35. **步骤条不许画百分比，未知不许染红**（批次 D）：PTP 握手与 0x9209 读取都是单次阻塞往返，
    没有任何可线性化的进度来源。会话连上但描述符没读回来时显示「未知」，
    写成红色失败会诱导用户重连——而重连正是让相机端句柄全部失效的那个动作（已知坑 4、14）。
