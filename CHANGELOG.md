...
**能按的动作不再只有一行字（`AppLink` 补上静止态容器）**

用户报告：「可以按的按钮只有文字，看不出是按钮」。根因在一个组件上——
`AppLink` 只有 `defaultMinSize(48dp)` + `clickable`，没有底色、描边、下划线，
而本应用的 `primary` 是近黑（浅色主题）/近白（深色主题），与正文只差一个字重。
它有 **76 处调用、分布在 22 个文件**：传输页的「清空 / 重试这 N 项未完成 / 继续选照片 /
查看 / 编辑 / 分享」、确认框的两个按钮、向导的「没有二维码？」「取消连接」、
相机档案的「导入」、相册页的「选择 / 全选 / 取消选择」……所以修一个组件就全部生效，
没有动任何调用点。

- 容器改成由 `color` 派生的一层色片：`Radius.Control` 12dp 圆角 + 浅色底 10% /
  深色底与深色浮层 22% / 按下 17% 与 30% / 禁用 5%。与 `AppButton` 同族但更轻，
  浓度压在 PRIMARY 之下（§2 原则 2）。**不加描边**——沿用 `AppButton` 玻璃路径
  「黑线描边会把极简版面切碎」的既有口径，层级只靠浓淡、尺寸、字色
- 两档浓度是截图里逼出来的：只按内容色亮度判断时，深色主题确认框里 `error` 红字那枚
  比旁边的「取消」更淡，破坏性动作反倒最不显眼。改成「`surface` 是深色 **或** 内容色是浅色」
  两个信号取其一，浅色卡片 / 深色主题 / 纯黑监看浮层三种场景一次覆盖
- 验收（AVD API 37 / 420dpi，浅色与深色各一遍）：传输页（页头「清空」+ 批量卡两枚 +
  任务行「查看/编辑/分享」三枚并排）、确认框（取消 / 继续）、连接向导（知道了 /
  没有二维码？/ 取消连接）、扫码页（上一步）、相机档案（导入，含禁用态「存为预设」）、
  相册页空态（连接相机）。**无一处裁字或换行**；页头量到标题右边界 936px、动作区从
  969px 起，两枚四字词（约 112dp/枚）放得下
- 任务行是 `run-as` + `sqlite3` 造的 4 条 seed 行（DOWNLOADING / DONE±savedUri / FAILED），
  截图后已删除，`download_task` 与 `download_history` 均为 0 行
- 字体缩放 200% 再量一遍（§9 清单要求，批次 B 的两个缺陷都是这一档才现形）：任务行三枚
  并排右边界 761px < 内容区 1036px，页头「清空」与标题不重叠，批量卡那枚 633px 也不裁
- **未验证**：监看与大图页面上 `OnViewer` 浮层的色片实际观感——没有索尼相机可连，
  取流画面进不去。该分支与深色主题走同一条浓度规则，深色主题一侧已实测
- 规范同步：`docs/UI-SPEC.md` §6.1 决策表把这一档改称「比按钮轻、但仍是能按的动作」，
  新增 §6.3「可点击的东西必须在静止态就看得出可点击」与提交前清单一条
- 校验：`./gradlew test :app:uiSpecCheck :app:lintDebug :app:assembleDebug` 全绿
  （全模块 282 项测试、`:app` 187 项，0 失败；`uiSpecCheck` 未发现裸 M3 控件）。
  外观没有可单测的纯函数，这一批的证据是上面那组截图，不是测试计数
- `AppButtonType.GHOST` 在 `feature/` 下是 0 使用，本次未动；若将来启用，同样的
  静止容器规则要一并带过去

## [0.2.0-alpha07] - 2026-09-26

> 页面与项目结构重构（批次 C 第二刀 → R）：传输页改为「重启后仍看得见、还能重试」，
> 连接流程收成带出口的向导，遥控与四个编辑器统一骨架，查看器改用媒体指纹定位；
> 并按两份规范逐条清掉了界面上「说没做过的事」的地方。
> 相机往返行为仍按机型证据表标注，未实测的一律不写成可用。

### Added / 新增

**批次 R（T2 第一段：曝光辅助的分析核心 + 节流门 + 监看工作台开关）**

差距文档 T2 明确说这部分「可独立于协议研究」，所以没有相机也能做、也就能验收：

- 新增 `image/ExposureAnalysis.kt`（纯 Kotlin，不碰 Bitmap/协程/GPU）：Rec.601 亮度直方图
  （256 桶 + 采样数）、高光占比（斑马纹依据）、一阶差分峰值密度（峰值对焦依据）、
  以及把长边降到 ~320px 的采样步长。**算法不进 ViewModel** 是 T2 点名的要求
- 新增 `image/FrameGate.kt`：T2 的三条硬约束（停用零分析、≤5Hz、不得形成无界队列）
  集中成一个可注入时钟的类型。时钟可注入所以能在 JVM 上验收——20fps 一秒 20 帧
  断言「最多 5 帧被分析」，这种话写在 ViewModel 里就没人能证明
- 接进监看工作台：工具栏一颗光圈图标开关辅助，开着才显示读数
  「高光 x.x% · 平均亮度 n」，没画面时显示「还没有可分析的画面」。
  分析就地跑在采集协程（`Dispatchers.Default`）里，像素缓冲按帧尺寸复用不每帧新建
  （960×640 的 IntArray 是 2.4MB，每秒分配 5 次就是给 GC 打工）
- 措辞守 T2 的边界：预览色域/伽马未知，所以读数只说「显示辅助」，不写 EV、不写"准确"、
  不碰"专业校准"那类表述
- 13 条新单测（合成灰阶/色块 + 一个朴素浮点参考实现对照）。
  写测试时参考实现先用 `toInt()` 截断，结果在整数灰阶上比实现差 1——那是**参考实现的错**，
  改成四舍五入后两边一致；`sampleStride(641,400)` 也从我猜的 2 改成算出来的 3

**这条还没满足 T2 的全部约束**：文档要求「持续持有 GPU 资源，不通过 Bitmap 往返 GPU」，
现在走的是 `Bitmap.getPixels` 的 CPU 回读。当前管线本来就是 CPU 解码 JPEG→Bitmap，
所以没有**新增**一次 GPU↔Bitmap 往返，但也没有做到文档设想的常驻 GPU 采样。
真不正确与否要由 T2 的验收决定：**同一真机同一取景，开/关各跑 10 分钟**，记录帧耗时、
丢帧、内存趋势与热状态——这需要相机，因此**未验证**。工作台上的开关与读数
也未经真机点击（无相机时进不去监看页），只有分析核心与节流门是单测证明过的。
...
- 新增 `data/transfer/DownloadLocation.kt`：**同一个偏好键 `"download_tree_uri"` 此前被四处硬写**
  （设置页读写、`CameraRepository` 两处写盘、照片编辑器入库）。漏改一处的表现是
  「设置里换了目录，但某条路径还往老地方写」——只有真机、且只有换过目录的用户会撞上。
  现在键、读写、给人看的那句「默认：DCIM/Imagedge（系统相册）」都只有一份
- 选择照片的底栏在**按下之前**就说清东西会存到哪儿（`保存到 …`）。原来这句话只在设置页里，
  第一次保存的人既不知道文件落在哪、也就不知道去哪儿确认它成功了（手册 §3 明令
  「不能为了短文案省去保存位置」）
- 批次摘要补「继续选照片」：这批传完了也有一条往前走的路，不用自己想在哪个 Tab
- 查看器给 RAW 单独一个标记「RAW 内嵌预览（非原片解码）」：屏幕上这张是 RAW 里内嵌的 JPEG，
  不是原片解码结果——只有「相机预览 / 已保存到手机」两种标记时，它看起来就像原图
- 存储卡确认读完了是空的，才有「存储卡里没有照片」这句；读失败/超时/被中断都不置该标记，
  仍走原来的说明。把「没读到」讲成「卡里没有照片」是手册点名的另一类谎

实测（AVD）：批次摘要显示「已保存 1 项，0 项未完成 / 继续选照片」；设置页保存位置一行
在改造后照常显示默认目录；185 单测、`uiSpecCheck`、`lintDebug` 全绿。
**未验证**：底栏的「保存到 …」（要相机内容才进得了选择态）、RAW 标记、空卡文案——
都需要真实相机；`DownloadLocation` 的换目录行为没有相机也无法走完整条写盘路径。

**批次 O（遥控页的手册缺口：措辞归位、盲拍要说明、参数要有白话解释 — 新手手册 §4 / §6）**

- `CaptureAvailability` 的 13 句中文判定说明全部搬进 `strings.xml`：`AvailabilityLine` 现在带
  `@StringRes noteRes` 而不是成品句子。判定是纯函数、跑在 JVM 单测里，把文案写死在里面
  等于让业务逻辑顺手 owns 措辞——改一句话要动判定代码，改版和翻译也没有落点。
  单测的断言也随之从「句子包含某个词」改成「是哪一条资源 id」，**更严**：它钉的是原因，
  不是措辞，改文案不会再误红
- **画面停了但快门还能用时，明说「看不到实时画面，仍可拍摄」**（手册 §4 原句）。
  原来三条状态各说各的，用户读到「取景已暂停」就以为整屏都停了，于是去重连——
  而那正是最不该发生的动作（重连让句柄全失效）。这条判定跨两件能力，写在 `captureAvailabilityOf`
  的出口处，并有单测锁住（快门正常时**不**撒这句）
- 参数面板每一行下面加一句白话解释（照相模式 / ISO / 光圈 / 快门 / 白平衡 / 曝光补偿），
  **不可调时也照样显示**：那时候用户更需要知道这项是干什么的。手册 §4「参数旁给短解释」
- 拍后落盘的三句结果文案改掉抢跑：入队不再说「已自动保存 N 张」（那时一张都没进相册），
  改说「已加入下载队列 N 张，存进相册后会在传输页显示」；自动保存没开时不再只写一个状态括号
  「（自动保存未开启）」，而是给出手册要的下一步——照片还在相机上，去「发送到智能手机」选片

实测（AVD，无相机）：参数面板六行的解释与禁用原因同时显示、无重叠；三件能力条的文案改由资源
提供后渲染不变；185 单测（+1 条盲拍判定）、`uiSpecCheck`、`lintDebug`、`assembleDebug` 全绿。
截图时临时放开过工作台那条「未连接不进遥控」的导航入口，截完已还原（`git diff` 对该文件为空，
全仓无 `TEMP PROBE`）。**未验证**：真有画面时「仍可拍摄」那句的出现时机、BLE 通路下的快门文案、
以及 `CameraControlViewModel` 里其余十几句硬编码中文（同属文案归属问题，这次只动了改到的三句）。
