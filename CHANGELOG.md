...
**批次 N4（新手手册 §3 / §5 / §6：会丢东西的决定、召回引导、全选）**

- 新增 `ui/components/ConfirmDialog.kt`（从相机档案页那份私有的确认框提上来，两处共用一套观感）
- 传输页三个「按下就会少掉一些东西」的动作不再直接执行：**全部取消 / 清空 / 清空记录**
  各过一个确认框，正文说清各自的实际影响（取消只停任务不动已存文件；清空只去掉列表条目；
  清记录不动相册原片）。原来这三条都是点一下就生效，而「清空」这个词本身
  分不出会不会碰到照片——用户不该靠猜
- 「最近连接」的清空同样补上确认（正文写明档案与预设不受影响）
- 设置页补 **使用帮助** 一行 → `GuidanceStore.reopenGuides()`：关掉的引导终于有第二条路找回来
  （新手手册 §5 要求的，之前只有 `reopenGuides()` 这个方法，没有任何调用点）
- 相册选择态补 **全选 / 取消全选**：作用对象是**当前筛选出来的这一批**，不是整个相册
  （筛成 RAW 后点「全选」却把 JPEG 勾上，界面说的就不是用户按下的那件事）。
  按钮文字随当前状态在「全选 / 取消全选」之间切换
- `GuidanceStore` 删掉 `markTaskSucceeded` / `hasSucceeded` 与 `guide_succeeded` 集合：
  三个方法**零调用点**，而「有没有成功存下照片」这件事传输历史表里已经有了
  （`download_history` 非空即成立）。在偏好里再存一份布尔就是第二个真相源，
  两边不同步时界面会在用户已经传成几十张之后继续给他看「第一次怎么传」。
  手册原本要求两个事实分开存——分开是对的，但**不该由这个存储来持有其中一件**

实测（AVD）：清空弹确认且确认后列表仍在（未点「继续」前一条都没删）；设置里「使用帮助」
点击后出现「已重新打开引导…」的回执；184 单测、`uiSpecCheck`、`lintDebug`、`assembleDebug` 全绿。
**未验证**：全选的真实勾选（要相机内容才能出网格）、引导被 `reopenGuides` 召回后在各页的复现、
边框水印/三拼/视频转动态三个编辑器的确认框观感（同一组件，但没走到它们的页面）。

**批次 N3（离开编辑器要确认 + 预览把工具挤出屏幕的老问题）**

- `EditorFrame` 补上设计 §2 与新手手册 §5 都要求、但一直没做的一条：**带着没存盘的改动离开
  要先问**。左上角返回、系统返回键、返回手势三条路径走同一个判断（此前全仓 `BackHandler`
  零命中，只有导出期间挡着返回）；没改过就不弹——每次都弹一个框，用户学会的是随手点掉，
  那时它什么都保护不了
- `EditorFrameState.hasEdits` 与既有的 `canReset` **分开**：三拼的「改动」是选好的三张素材，
  而它没有「重置」这一档；导出完成页产物已落盘，离开也不再问。四个编辑器各自接入
  （编辑调节 / 边框水印用现成的 `state.hasEdits`，三拼看槽位与阶段，视频转 LIVE 看会话是否还在）
- 验证时撞出一个**比 N3 更早**的布局问题：编辑调节里预览是 `fillMaxWidth().aspectRatio(照片比例)`，
  竖图会把整屏吃掉——实测 1080×2400 上预览占 290..1992，调色/裁剪/旋转三个 Tab 落在 2251，
  **第一屏完全看不到工具**。这不是批次 H 引入的（H 之前 `AppPage` 同样是滚动列，行为一致），
  是这一页一直就有的问题。现在预览高度上限取屏高 42%，且**上限落在预览块自己身上并保持照片比例**：
  裁剪框是按这个 Box 的像素换算的，改成固定高度让照片 letterbox 就会裁偏
- 中途一次错误尝试已回退：先试了 `height(固定)` + `ContentScale.Fit`，工具是露出来了，
  但裁剪框与画面对不上；也试过 `heightIn(max)` 配默认 `aspectRatio`，实测根本没夹住

实测（AVD）：选图 → 点一个滤镜 → 返回，弹出「还没保存，确定离开？」；「继续编辑」留在原页
且滤镜选择还在，「放弃修改并离开」回到创作首页；预览 473×1052px（= 照片自身比例）居中，
工具区第一屏全部可见；没改动时返回**不弹框**直接走。184 单测、`uiSpecCheck`、`lintDebug` 全绿。
**未验证**：裁剪分区在新预览尺寸下的手感与边框水印/三拼/视频转动态三个编辑器的首屏布局
（同一类问题，但这台机器上只有编辑调节能走通到出图）。

**批次 N2（两句谎话：指向不存在按钮的指引，和大小未知时的 0%）**

- 工作台帮助面板第二步原文是「点『扫码连接』，手机自动配网」——**那个按钮在批次 B/D 就删掉了**，
  现在这一屏唯一的入口叫「连接相机」。批次 D 收口时漏了这条文案：新手照著找，找不到，
  只会怀疑是自己操作错。改为「回到手机点『连接相机』，按向导扫码自动配网」
- 同一批第一步原来写「相机进入『智能手机连接』」，把机型差异抹平成一条统一菜单路径
  （新手手册明令不得伪造）。改为按机型说明叫法不同，并给出可执行的找法：
  **在相机菜单里搜「智能手机」**
- 任务行的状态文字改为读 `strings.xml`：大小已知的下载中显示「传输中 N%」，
  大小未知显示「传输中（大小未知）」——原来这一行直接打 `"${progress}%"`，
  与同一张卡片上方那条刻意改成不定长的进度条互相矛盾（批次 C 只修了条、没修字，
  `CHANGELOG` 里那条"已修完"的说法已在原处补正）。顺带把「完成」改成「已保存」，
  说清它说的是东西落到相册里了，不是"这个任务跑完了"

实测：184 单测全绿；文案改动在 AVD 上核对了帮助面板与任务行的渲染。**未验证**：
真机上帮助面板的换行与长度（AVD 只能看一种屏宽）。
...
| 状态 | 结果 |
|---|---|
| 工作台（未连接） | 浅/深都正常，玻璃状态卡与四标签胶囊不重叠 |
| 连接向导（相机准备步） | 三步头 + 引导卡 + 「二维码已显示 / 没有二维码 / 取消连接」三条出口都在 |
| 相册（未连接空态） | 只给「连接相机」一条路，范围行不出现；200% 字体下按钮仍完整停在导航胶囊之上 |
| 传输（部分失败的一批） | 摘要「已保存 2 项，2 项未完成 / 重试这 2 项未完成」；完成行给查看/编辑/分享，失败行给原因 + 重试；200% 字体动作行不裁切 |
| 传输记录 | 有 `savedUri` 的两条给「查看」；老记录（无 handle、无 uri）两条都不给——不假装能打开，也不知道怎么重试 |
| 全局任务条 | 重启后仍弹（证明是 Room 驱动而非内存），进传输页看一眼即消失且 `failureViewed` 落库 |
| 创作首页 + 四个编辑器选素材页 | 正常，都没有灰掉的「保存副本」 |
| 设置 | 分组正常，主题切换立即生效 |

- **测出一个真 bug 并修掉**：冷启动进传输页，批次摘要**永远在屏幕上方看不见**。摘要是列表第一项，
  而它的 flow 初值是 `null`——第一帧没这一项、下一帧才插进来时，LazyColumn 把视口锚在当时的
  第一项（传输策略）上，新插的摘要就被顶出屏幕。现在初值直接由队列当前值算出
  （`TransferViewModel.batchSummary`），冷启动第一帧就有数字。这条是**看截图才发现的**，
  单测与 lint 都不会报它
- 未验证（没有索尼相机可连，按规矩不拿模拟器假数据当协议证据）：相机已连的工作台与相册网格、
  选择态底栏、缩略图独立重试、范围行的传输原因、查看器翻页/缩放、遥控有画面时的取景比例与
  BLE 半按松开、语音读屏选片（读屏要人耳确认；代码侧的 `Role.Checkbox` / `selected` /
  `contentDescription` 已按 §8.6 建好）

**批次 K（遥控页迁到沉浸页骨架 — 设计 §4.6 / §8.3）**

- 新增 `ui/layout/ImmersiveFrame.kt`（含 `immersiveLayoutOf` 纯函数 + 5 条单测）。
  它与 `AppScreenFrame` 的分工就一条：普通页把 inset 一次分给顶栏和内容，因为内容是列表；
  沉浸页的内容是**画面**，画面该吃掉能吃的空间，inset 是控件层的事。
  竖屏画面上、控件下；横屏（宽 > 高且 ≥ 600dp）画面左、控件右；方形窗口（折叠屏展开、分屏）
  继续上下排——两栏都会被压窄
- 遥控页是最后一个还在自己拼 `Scaffold + 旧 PageHeader + 滚动列` 的页面，现在也没了；
  整列内容不再包在 `GlassCard` 里（§3.2「长列表默认普通表面」）
- 排布判定改过两处，都是**先截图发现再改**、不是推演出来的：
  ① 画面按宽度定 3:2，横屏会顶出槽外压住上面的状态行 → 骨架把当前排布交给画面，
  横屏改按高度定 3:2；② 三行状态留在标题下时横屏只剩半屏高给画面 → 标题收小、
  状态行移到控件栏顶部（那里同样是「按快门之前先看的东西」，但不占画面高度）
- 快门与录像并排（设计 §3.3 线框）。相机从没回报过录像状态时这一栏只有快门，改居中——
  按 §4.6「不用本地布尔值猜测」，宁可不摆录像按钮
- 快门提示改成**有条件**且**分通道**（§4.6 + §5）。原来那行「按住对焦 · 松开拍照」永久显示，
  而没连蓝牙的通道按下就是单次触发（`shutterDown()` 直接 `captureNow()`），
  这句话在承诺一个不存在的机制。现在：按「机型 + 操作模式」记引导状态（换机器、换模式都重新说）、
  认出相机之前不说、点「知道了」后不再说；BLE 说「按住对焦 · 松开拍照」，
  另一条通道说「点一下即拍摄（这条通道没有半按对焦）」
- 截图说明：无相机时遥控页从工作台进不去（工作台会把「用手机遥控」导去连接向导，
  这是批次 D 的正确行为），所以这两张图是临时放开那一行导航入口截的，截完已还原
  （`git diff` 对 `CameraHubScreen.kt` 为空，全仓无 `TEMP PROBE`）

实测（AVD，无相机）：横屏画面 1212×808px 恰为 3:2、不再压住状态行，控件栏可滚、
72dp 快门完整可点；未认出相机时提示那一行不出现。183 单测、`uiSpecCheck`、`lintDebug`、
`assembleDebug` 全绿。**未验证**：真有画面时的取景比例、BLE 连上后的提示文案、
录像按钮与快门并排的样子（这批没有碰快门时序代码）。

**批次 J（相册页：切换判定、结构化说明、单格重试 — 设计 §4.3 / §8.2 / §8.5）**

- 新增 `feature/photos/ScopeSwitch.kt`：**能不能为了切范围去改相机功能模式**做成纯函数
  （`channelGateOf` + 5 条单测）。原来这条规则在两处各写一遍字面量、措辞还不一样
  （一处「浏览范围」一处「浏览通道」），判定本身也散在 `switchScope` / `enter` 两个协程里。
  现在 `switchScope` 在**抬手时**和**真的动通道之前**各判一次（§8.2「不能只靠按钮禁用保证安全」：
  协程排到这里时用户可能刚点了下载）。没连接单独判为 `NoSession`，不再混进「切换失败」——
  两者的修复路径不同，一个是去连，一个是等待或重试
- `_error: String?` 换成 `PhotosNotice`：ViewModel 只说「是什么事」，措辞回到 `strings.xml`
  （原先 4 条中文硬编码在 VM 里），补救动作**按事给**——入队失败是「再存一次」（选择还在）、
  未连接是「去连接」、切范围失败是「再切一次那个目标」、重连失败是「再连一次」。
  以前四种都画成同一个「重试 → 重新加载列表」，等于把四件不同的事按成第五件
- 范围行把「传输中不能切」的原因写在**行下面**（设计 §2），不用打开面板才知道不行；
  行本身仍可点，因为用户有权看清为什么现在不能切。相机已在目标模式时最坏是多说一句，
  不会拦掉本该允许的切换
- 缩略图独立重试（§4.3「不让一张坏图变成全页错误」）：新增失败集合，坏格自己带「重试」。
  顺手补上 `loadThumbnail` **原来没有的 catch**——一次通道异常会逃到 `viewModelScope`
  把整个相册页带走，滚动时最容易撞上；取消不当失败记（否则给一个没坏的东西标上重试，
  还把离页取消伪装成通道故障）；缓存被 trim 或切范围清过之后按「缓存代数」复位失败集合，
  格子重新获得自动加载的机会
- 后台轮询失败现在会说**一次**（§4.3 末段）：同一轮故障不每 4 秒重复打扰，扫成功过一次就把
  横幅收回，且只收回自己那条（用户可能还在读「入队失败」）；已有内容一律不清空
- 设计 §8.2 的 `CameraAccess` 汇总结构这一批**没有**照搬：它九个字段里当下只有切范围这三条
  有真实调用方，画面/快门/拍后保存已经各有自己的纯判定（`captureAvailabilityOf`）。
  再造一个汇总结构只会多一处需要同步的地方，等出现第二个跨页消费者时再建

实测：AVD 上相册页重构后渲染正常（未连接空态 + crash buffer 干净），178 单测、
`uiSpecCheck`、`lintDebug`、`assembleDebug` 全绿。范围行的切换原因、单格重试、
后台失败横幅都要有真实相机内容才能触发，**未验证**（无索尼相机可连）。
...
- 新增 `ui/layout/EditorFrame.kt`：设计 §4.7 的编辑器骨架（返回/标题/重置 → 预览 → 工具区 →
  「保存副本」）。三条规矩由骨架统一保证，不再靠每个编辑器各自想起来：
  ① 导出期间**返回键不起作用并写明原因**——这个项目的导出没有安全取消点（Media3 转码与
  PTP 写入中途丢下会留半成品），所以给一个「取消」按钮就是撒谎；
  ② 「重置」过确认对话框（UI 规范 §5：会丢失成果的决定要确认），且只在真的
  有可撤销改动时可点；③ 结果说明常驻在内容上方，成功与失败同一个位置
- 「编辑调节」先迁过去（§9：每批先做一个代表页面，其余再迁移）。
  另外三个编辑器（边框水印 / LIVE 三拼 / 视频转动态）仍是旧骨架，排在下一刀
- 对比原图补了**显式按钮**（原来只有长按）。长按不可发现，读屏也拿不到（设计 §4.7）
- 删除随骨架失效的 `edit_save` / `edit_exporting` 两条字符串
...
- **三拼的骨架主按钮和阶段主按钮撞车**（批次 H 迁入 `EditorFrame` 时留下，批次 I 逐个打开编辑器
  才看见）：`saveVisible` 默认 true、`onSave` 又固定是 `export()`，于是选图页多一个灰掉的
  「保存副本」；阶段一同时出现「保存副本」与「进入拼接预览」两个下一步，点前者会**跳过预览直接导出**；
  完成页还能再点一次「保存副本」，在同一批素材上再生成一份。`export()` 只校验「三张素材齐了」，
  不校验阶段，所以这三条都会真的执行出去。现在骨架的主按钮写的就是当前那一步（`saveLabel`），
  阶段里重复的主按钮删掉，选图页与完成页不出现主按钮，导出期间文案让回「正在生成…」。
  实测（AVD）：选图页只剩「选择实况图」一个动作。阶段一/阶段二/完成页**未实测**——三拼要 3 张实况图，
  而这台 AVD 的 photo picker 列不出任何视频（MediaStore 里有 `src.mp4` 也不显示）、相机又不可用，
  造不出实况图输入；真机上走一遍三个阶段才算验证完。
...
- **大小未知的任务挂着一条永远 0% 的进度条**（批次 C）：`progress` 只在 `total > 0` 时才计算，
  ...
  （**批次 N2 补正**：当时只改了进度条，右边那行状态文字仍然直接打 `"${progress}%"`，
  于是同一张卡片上一边说不定长、一边写 0%。文案归属那一条漏了）
